...
var Windy = function Windy(params) {
  ...
  */
   ...
   */
	...
	var animationLoop;
	var animate = function animate(bounds, field, oldParticles) {
		...
		for (var i = 0; i < particleCount; i++) {
			if(oldParticles.length){
				particles.push(oldParticles.shift());
			}else{
    ...
    var ttt = new Date().getTime();
		...
		buildGrid(gridData, function (grid) {
			...
			// interpolateField
			interpolateField(grid, builtBounds, mapBounds, function (bounds, field) {
				// animate the canvas with random points
				stop();
				windy.field = field;
				windy.particles = animate(bounds, field, windy.particles);
			});
		});
	};
